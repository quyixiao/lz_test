...
public class TestInterfaceServiceImpl implements TestInterfaceService {
    ...
    public R doAddOrUpdate(InterfaceReq req) {
        ...
        if (testInterface != null) {                                  //是否在解决冲突
            R r = gitOperation(branch, directory, testInterface.getGitFileName(), req.getCode());
            if (NumberUtil.objToIntDefault(r.get("code"), 0) != 1000) {
                return r;
            }
    ...
    public R gitOperation(String branch, String directory, String fileName, String code) {
        try {
            ...
            if (MASTER.equals(currBranch)) {          // 如果当前分支是 master分支
                ...
                git.checkout(branch);
                git.pull();
            } else if (branch.equals(currBranch)) {        //如果当前分支是自己之前所在分支，可能是解决冲突
                //什么都不做
            ...
            // 写入文件
            git.write(directory, fileName, code);
            // 提交推送到远程
            git.add(directory + "/" + fileName);
            git.commit("提交本次修改");
            // 合并 master 文件
            String status = git.merge(MASTER);
